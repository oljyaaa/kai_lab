...
$("download").onclick = () => {
  const payload = result || {
    type: "glitter-fpv-live-snapshot",
    note: "Це поточний snapshot. Для повного вимірювання натисни «Виміряти 10 секунд».",
    mode,
    browser: navigator.userAgent,
    dpr: window.devicePixelRatio,
  ...
  a.href = url;
  a.download = `glitter-fpv-${payload.mode || mode}.json`;
  a.hidden = true;
  ...
  $("status").textContent = result
    ? "Файл із повним 10-секундним вимірюванням завантажено."
    : "Завантажено поточний snapshot. Для повного звіту запусти вимірювання на 10 секунд.";
};
