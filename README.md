...
Керування обирається у «Пульті керування»: **WASD** (W — тяга, A/D — поворот, S — гальмування), **стрілки** (↑ — тяга, ←/→ — поворот, ↓ — гальмування) або **екранні кнопки** з Pointer Events і pointer capture. Неактивна схема не керує дроном. **R** повертає на старт. Для польоту після роботи з кнопками клікни арену; Tab лишається стандартною навігацією між елементами інтерфейсу.
...
`npm test`: 7 тестів — чистота фізики/ліміт швидкості; wrap та кутова інтерполяція; однакові 300 steps на змодельованих 60/120/144 Hz; clamp і pause/resume; input/повтор/blur/cleanup; пріоритет гальма над тягою; ізоляція схем керування.
