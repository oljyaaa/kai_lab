...
У картці «Візуальна лабораторія» увімкни «Привид маршруту», щоб побачити останні 10 секунд польоту, або «Показати інтерполяцію», щоб порівняти `previous`, `current` і проміжний стан `render (α)`. JSON тепер завантажується завжди: до вимірювання — поточний snapshot, після 10 секунд — повний масив результатів.
