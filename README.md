...
Шрифти Comfortaa/Nunito завантажуються з Google Fonts. Без інтернету гра використовує системний sans-serif. Всі ігрові фігури малюються Canvas без зовнішніх спрайтів.
...
`endStep()` очищає лише одноразові натискання після кроку симуляції. Автоповтор клавіатури не створює повторного `justPressed`. `blur` очищає клавіші, `destroy()` видаляє слухачів. Обробка не перехоплює клавіші у кнопках, полях вводу та select.

Керування: **W / ↑** — тяга; **A / ←** та **D / →** — поворот; **R** — повернутися на старт. Є кнопки паузи й скидання, а також екранні кнопки з Pointer Events і pointer capture для сенсорного керування.
...
`npm test`: 5 тестів — чистота фізики/ліміт швидкості; wrap та кутова інтерполяція; однакові 300 steps на змодельованих 60/120/144 Hz; clamp і pause/resume; input/повтор/blur/cleanup.
